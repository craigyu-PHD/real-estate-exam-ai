...
export function ExamCountdown() {
  ...
  return (
    <div className="bg-gradient-to-br from-indigo-600 via-violet-600 to-indigo-700 rounded-[1.5rem] p-6 text-white relative overflow-hidden shadow-xl">
      ...
      <div className="text-xs opacity-80 mt-1">考試日：2027/11/06（六）09:00｜每天 10 分鐘穩穩推進</div>
      <div className="mt-4 h-2 bg-white/20 rounded-full overflow-hidden"><div className="h-2 bg-white rounded-full" style={{width: `${Math.max(6, Math.min(100, 100 - diff/5))}%`}} /></div>
    </div>
  );
}
