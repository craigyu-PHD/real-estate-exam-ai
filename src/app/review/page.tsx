...
export default function Review() {
  ...
  const cur=reviewList[idx], lawName=lawsData.find(l=>l.id===cur.lawId)?.name||cur.lawId;
  const articleText=generatedArticles[cur.lawId]?.find(a=>a.articleNumber===cur.articleId)?.text || '';
  const reviewSummary=articleText.replace(/^第\s*[^條]+條\s*/, '').slice(0,90) || '回想這一條的主體、條件與法律效果。';
  ...
  return <div className="page-shell max-w-3xl space-y-5">
    ...
    <section className="card rounded-[1.5rem] p-6 md:p-8"><div className="flex flex-wrap gap-2"><span className="surface rounded-full px-2.5 py-1 text-[10px] font-bold text-tertiary">{lawName}</span><span className="surface rounded-full px-2.5 py-1 text-[10px] font-bold text-tertiary flex items-center gap-1"><Clock3 size={10}/>間隔 {cur.interval} 天</span></div><h2 className="text-3xl font-black mt-5 text-primary">第 {cur.articleId} 條</h2><p className="text-lg font-black leading-relaxed mt-3 text-primary">{reviewSummary}</p><div className="surface rounded-2xl p-4 mt-5"><div className="text-[10px] font-black text-tertiary">法條提示</div><p className="text-sm leading-loose mt-1 text-secondary">{articleText.slice(0,180)}{articleText.length>180?'…':''}</p></div></section>
