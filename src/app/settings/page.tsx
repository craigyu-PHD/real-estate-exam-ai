...
export default function SettingsPage() {
  ...
  return (
    <div className="page-shell settings-compact max-w-5xl space-y-3.5">
      ...
      <section className="card rounded-[1.4rem] overflow-hidden">
        <div className="px-4 py-3 border-b flex items-center gap-2" style={{borderColor:'var(--border)'}}><SlidersHorizontal size={16} className="text-indigo-600"/><span className="text-sm font-black text-primary">閱讀體驗</span></div>
        <div className="divide-y" style={{borderColor:'var(--border)'}}>
          <div className="p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl surface flex items-center justify-center text-secondary"><Type size={18}/></div><div><div className="text-sm font-black text-primary">閱讀字級</div><div className="text-xs mt-0.5 text-tertiary">法條、講解與題目同步調整</div></div></div><div className="flex gap-1 rounded-full p-1 progress-track border" style={{borderColor:'var(--border)'}}>{(['small','medium','large'] as const).map(size=><button key={size} onClick={()=>updateSettings({fontSize:size})} className={`px-4 py-1.5 rounded-full text-xs font-black transition ${settings.fontSize===size?'bg-indigo-600 text-white':'text-secondary'}`}>{size==='small'?'小':size==='medium'?'中':'大'}</button>)}</div></div>
          <label className="p-4 flex items-center justify-between gap-4 hover:bg-indigo-500/[0.025] cursor-pointer transition"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl surface flex items-center justify-center text-secondary"><Play size={18}/></div><div><div className="text-sm font-black text-primary">聽課自動連播</div><div className="text-xs mt-0.5 text-tertiary">Mini Lecture 播放完成後自動進下一條</div></div></div><input type="checkbox" checked={settings.autoPlayNext} onChange={e=>updateSettings({autoPlayNext:e.target.checked})} className="w-5 h-5 accent-indigo-600"/></label>
        </div>
      </section>
      ...
      <section className="card rounded-[1.4rem] overflow-hidden">
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-3">
            {([
            ...
            ]).map(provider => <div key={provider.id} className="surface rounded-xl p-3.5">
              ...
              <div className="relative mt-3"><input type={showProvider===provider.id?'text':'password'} value={apiKeys[provider.id]} onChange={e=>{setApiKeys(prev=>({...prev,[provider.id]:e.target.value}));setTestState({provider:null,state:'idle',message:''});}} placeholder={provider.placeholder} className="input-shell w-full rounded-lg px-3 py-2 pr-9 text-xs outline-none" autoComplete="off" spellCheck={false}/><button onClick={()=>setShowProvider(showProvider===provider.id?null:provider.id)} className="absolute right-1.5 top-1/2 -translate-y-1/2 icon-button !w-7 !h-7 border-0">{showProvider===provider.id?<EyeOff size={13}/>:<Eye size={13}/>}</button></div>
              {savedKeys[provider.id]&&<div className="mt-1.5 text-[9px] text-tertiary">{maskApiKey(savedKeys[provider.id])}</div>}
              <div className="grid grid-cols-2 gap-2 mt-3"><button onClick={()=>void testProvider(provider.id)} disabled={testState.state==='testing'} className="bg-indigo-600 text-white rounded-lg py-2 text-[10px] font-black disabled:opacity-50">{testState.provider===provider.id&&testState.state==='testing'?'測試中…':'儲存＋測試'}</button><a href={provider.href} target="_blank" rel="noreferrer" className="rounded-lg py-2 text-[10px] font-black surface text-primary flex items-center justify-center gap-1">取得 Key <ExternalLink size={11}/></a></div>
            </div>)}
          </div>
