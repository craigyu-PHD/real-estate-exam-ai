...
export default function ExamsIndex() {
  ...
  return (
    <div className="page-shell max-w-3xl space-y-4">
      <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><span className="question-number-badge">Q {idx+1}</span><span className="text-xs font-black text-secondary">{examMode==='formal'?'模擬正式':lawName} · {typeLabels[q.type]}</span></div><span className="text-xs font-black text-tertiary">{idx+1}/{pool.length} · {score} 分</span></div>
      ...
      <section className="card rounded-[1.5rem] p-6 md:p-7">
        ...
        <div className="space-y-2.5 mt-5">{q.options.map((option,i)=>{const correct=show&&i===q.answer, wrong=show&&selected===i&&i!==q.answer;return <button key={i} onClick={()=>pick(i)} disabled={show} className={`exam-option w-full text-left p-3.5 rounded-xl border flex items-center justify-between gap-3 text-sm font-bold transition ${correct?'bg-emerald-500/10 border-emerald-500/40 text-emerald-700 dark:text-emerald-200':wrong?'bg-rose-500/10 border-rose-500/40 text-rose-600 dark:text-rose-200':'surface text-secondary hover:border-indigo-400'}`}><span className="flex items-center gap-3"><span className="option-letter">{String.fromCharCode(65+i)}</span><span>{option}</span></span>{correct&&<Check size={16}/>} {wrong&&<X size={16}/>}</button>;})}</div>
